...
print("   • 선택 2 [드래곤 이동] : 이미 부화한 드래곤을 다른 칸으로 이사시킵니다.)")
        ...
        # --------------------------------------
            ...
            if (0 <= s_row <= 2 and 0 <= s_col <= 2) and (0 <= e_row <= 2 and 0 <= e_col <= 2):
                if '🐉' in game_map[s_row][s_col] and '[🥚]' not in game_map[e_row][e_col]:
                    ...
                    if gold >= 500:
                        gold -= 500
                        print("💸 자산이 500 Gold 차감되었습니다.")
                    else:
                        print(f"💸 자산이 부족하여 가진 돈 전액({gold} Gold)이 차감되었습니다.  10년 전에 보증을 서 준 친구에게 연락해 빚은 면했습니다. ")
                        gold = 0
